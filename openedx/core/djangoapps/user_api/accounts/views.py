...
class AccountView(APIView):
    ...
    **Response Values for GET**
        ...
        If the user makes the request for her own account, or makes a reque st
        for another account and has "is_staff" access, the response contains:

        * username: The username associated with the account.

        * name: The full name of the user.

        * email: email for the user (the new email address must be confirmed via
          a confirmation email, so GET will not reflect the change until the
          address has been confirmed).

        * date_joined: The date the account was created, in the string format
          provided by datetime. For example, "2014-08-26T17:52:11Z".
        ...
        * country: null (not set), or a Country corresponding to one of the ISO
          3166-1 countries.
        ...
        * bio: null or textural representation of user biographical information
          ("about me").
        ...
        * profile_image: JSON representation of a user's profile image
          information. The keys are: the user's profile image:
            ...
            * "image_url_*": absolute URL to various sizes of a user's profile
              image, where '*' matches a representation of the corresponding
              image size such as 'small', 'medium', 'large', and 'full'. These
              are configurable via PROFILE_IMAGE_SIZES_MAP.

            * requires_parental_consent: true if the user is a minor requiring
              parental consent.
        ...
        For all text fields, clients rendering the values should take care to
        HTML escape them to avoid script injections, as the data is stored
        exactly as specified. The intention is that plain text is supported, not
        HTML.
    ...
    **Response Values for PATCH**
        ...
        user does not have username "username", this method will return with a
        status of 403 for staff access but a 404 for ordinary users to avoid
